...
import { onChangeSkipNumber, setProductsByPage } from "../redux/FilterProductsReducer";
...
const ProductsPage = () => {
  ...
  const skipProducts = useSelector((state: RootState) => state.filterProducts.filters.skipNumberForPage)

  const products = useSelector((state: RootState) => state.filterProducts.products)
  ...
  useEffect(() => {
    if (!isFetching && data) {
      dispatch(setProductsByPage(data))
  ...
  return (
    ...
    <div className="flex">
      {page.map((page, id) => <div key={id} onClick={() => onChangePage(id * 10)}>{page}</div>)}
    </div>
    </>
  );
};
